...
// ======== 可調參數（更穩健，不依賴 realpath） ========
$CAR_IMAGE_DIR_FS = __DIR__ . '/../../assets/imgs/cars/'; // 伺服器檔案系統
...
$CAR_IMAGE_DIR_FS_REAL = realpath($CAR_IMAGE_DIR_FS) ?: $CAR_IMAGE_DIR_FS;
  ...
  switch ($action) {
    ...
    case 'list': {
      $stmt = $conn->prepare("
        SELECT vehicle_id, license_plate, vehicle_type, owner, user, tonnage, brand,
               ...
               crane_type, inspection_date, insurance_date,
               record_date, emission_date,
               insulation_test_date, xray_test_date,
               image_path, maintenance_times, maintenance_cost,
               record_required, emission_required,
               insulation_required, xray_required
        FROM vehicles
    ...
    case 'validate': {
      ...
      if ($type === 'plate') {
        ...
        $params = [':v' => $value];
        if ($currentId !== '') {
          // 以 vehicle_id 排除自己（舊前端 current_id 傳的是原始 vehicle_id）
          $sql .= ' AND vehicle_id <> :cid';
          $params[':cid'] = $currentId;
        }
        $stmt = $conn->prepare($sql);
      ...
      if ($type === 'file') {
        ...
        $target = $CAR_IMAGE_DIR_FS_REAL . DIRECTORY_SEPARATOR . $fileName;
    ...
    case 'upload_image': {
      ...
      $finalName = $orig;
      $dest = $CAR_IMAGE_DIR_FS_REAL . DIRECTORY_SEPARATOR . $finalName;
      if (file_exists($dest)) {
    ...
    case 'create': {
      ...
      [$insulation_test_date, $insulation_required] = normalize_optional_date($data['insulation_test_date'] ?? '');
      [$xray_test_date,       $xray_required]       = normalize_optional_date($data['xray_test_date'] ?? '');
      ...
      $sql = "
        INSERT INTO vehicles (
          ...
          inspection_date, insurance_date, record_date, emission_date,
          insulation_test_date, xray_test_date,
          image_path,
          record_required, emission_required, insulation_required, xray_required
        ) VALUES (
          :vehicle_id, :license_plate, :vehicle_type, :owner, :user, :tonnage, :brand,
          :vehicle_year, :vehicle_price, :truck_bed_price, :crane_price, :crane_type,
          :inspection_date, :insurance_date, :record_date, :emission_date,
      ...
      $stmt->bindValue(':insulation_test_date', $insulation_test_date);
      $stmt->bindValue(':xray_test_date',       $xray_test_date);
      ...
      $stmt->bindValue(':insulation_required',  $insulation_required, PDO::PARAM_INT);
      $stmt->bindValue(':xray_required',        $xray_required,       PDO::PARAM_INT);
      $stmt->execute();
    ...
    case 'update': {
      ...
      $sql = "
        UPDATE vehicles SET
          ...
          insulation_test_date = :insulation_test_date,
          xray_test_date       = :xray_test_date,
      ...
      if ($image_path && $oldImg) {
        ...
        if ($oldFs) {
          safe_unlink_in_cars($oldFs, $CAR_IMAGE_DIR_FS_REAL);
        }
      }
    ...
    case 'delete': {
      ...
      if ($stm2->rowCount() > 0 && $img) {
        ...
        if ($imgFs) {
          safe_unlink_in_cars($imgFs, $CAR_IMAGE_DIR_FS_REAL);
        }
      }
 ...
 * - 目前保險沒有 required 欄（schema 無），其餘四種檢驗皆有 required 與日期。
