...
// 小工具：日期顯示（所有可選檢驗：旗標為0或日期空/0000-00-00 → 顯示「不須檢驗」）
...
<div class="container-fluid my-4">
  ...
  <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
    <div>
      ...
      <span class="ms-5">
        把境宏小幫手拉進群組，每天0700向你回報各項檢驗狀況。
      </span>
      <span class="text-primary" style="cursor:pointer;" data-bs-toggle="modal" data-bs-target="#helperQrModal"><u>點我顯示QR</u>
      </span>
    </div>
    ...
    <table id="vehicleTable" class="table table-bordered table-striped table-hover align-middle text-center mb-0">
      <thead>
        <tr>
          ...
          <th class="th-2line"><span>絕緣測試</span><span>日期</span></th>
          <th class="th-2line"><span>X光檢測</span><span>日期</span></th>
          <th>操作</th>
        </tr>
      </thead>
      <tbody>
        <?php foreach ($vehicles as $i => $v): ?>
          ...
          <tr>
            ...
            <td><?= $showDate($v['insulation_test_date'] ?? null, (int)($v['insulation_required'] ?? 1)) ?></td>
            <td><?= $showDate($v['xray_test_date'] ?? null, (int)($v['xray_required'] ?? 1)) ?></td>
            <td>
              <button
                ...
                data-insulation_required="<?= (int)($v['insulation_required'] ?? 1) ?>"
                data-insulation_test_date="<?= htmlspecialchars($v['insulation_test_date'] ?? '', ENT_QUOTES) ?>"
                data-xray_required="<?= (int)($v['xray_required'] ?? 1) ?>"
                data-xray_test_date="<?= htmlspecialchars($v['xray_test_date'] ?? '', ENT_QUOTES) ?>">編輯</button>
              ...
              <button
                ...
                data-vehicle_id="<?= htmlspecialchars($v['vehicle_id'] ?? '', ENT_QUOTES) ?>"
                data-image-url="<?= htmlspecialchars($image_url, ENT_QUOTES) ?>">刪除</button>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
    ...
    <form class="modal-content" method="post" action="car_edit_backend.php?action=create" enctype="multipart/form-data" id="addVehicleForm" novalidate>
      ...
      <div class="modal-body">
        <div class="row g-2">
          ...
          <!-- 絕緣測試（須/不須 + 日期） -->
          <div class="col-12">
            <label class="form-label d-block">絕緣測試</label>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="insulation_required" id="add_insulation_yes" value="1" required>
              <label class="form-check-label" for="add_insulation_yes">須檢驗</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="insulation_required" id="add_insulation_no" value="0" required>
              <label class="form-check-label" for="add_insulation_no">不須檢驗</label>
            </div>
            <div class="mt-2" id="add_insulation_date_wrap"><!-- 動態插入 date --></div>
          </div>

          <!-- X 光檢測（須/不須 + 日期） -->
          <div class="col-12">
            <label class="form-label d-block">X光檢測</label>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="xray_required" id="add_xray_yes" value="1" required>
              <label class="form-check-label" for="add_xray_yes">須檢驗</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="xray_required" id="add_xray_no" value="0" required>
              <label class="form-check-label" for="add_xray_no">不須檢驗</label>
            </div>
            <div class="mt-2" id="add_xray_date_wrap"><!-- 動態插入 date --></div>
          </div>
      ...
      <div class="modal-body">
        ...
        <div class="row g-2">
          ...
          <div class="col-12">
            ...
            <div class="mt-2" id="edit_record_date_wrap"><!-- 動態 date --></div>
          </div>
          ...
          <div class="col-12">
            ...
            <div class="mt-2" id="edit_emission_date_wrap"><!-- 動態 date --></div>
          </div>
          ...
          <div class="col-12">
            <label class="form-label d-block">絕緣測試</label>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="insulation_required" id="edit_insulation_yes" value="1" required>
              <label class="form-check-label" for="edit_insulation_yes">須檢驗</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="insulation_required" id="edit_insulation_no" value="0" required>
              <label class="form-check-label" for="edit_insulation_no">不須檢驗</label>
            </div>
            <div class="mt-2" id="edit_insulation_date_wrap"><!-- 動態 date --></div>
          </div>
          ...
          <div class="col-12">
            <label class="form-label d-block">X光檢測</label>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="xray_required" id="edit_xray_yes" value="1" required>
              <label class="form-check-label" for="edit_xray_yes">須檢驗</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="xray_required" id="edit_xray_no" value="0" required>
              <label class="form-check-label" for="edit_xray_no">不須檢驗</label>
            </div>
            <div class="mt-2" id="edit_xray_date_wrap"><!-- 動態 date --></div>
          </div>
...
<!-- 境宏小幫手 QR Modal（美化版） -->
<div class="modal fade" id="helperQrModal" tabindex="-1" aria-labelledby="helperQrModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm modal-dialog-centered">
    <div class="modal-content helper-qr-card text-center">
      <div class="modal-header border-0 pb-0">
        <img src="<?= public_base() ?>/assets/imgs/JH_logo.png"
          alt="Logo"
          class="helper-logo-circle">
        <h6 class="modal-title fw-semibold mb-0" id="helperQrModalLabel">境宏小幫手</h6>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="關閉"></button>
      </div>
      ...
      <div class="modal-body pt-2">
        <!-- QR 外框 -->
        <div class="helper-qr-frame mx-auto">
          <img
            src="<?= public_base() ?>/assets/imgs/helper_qr.png"
            alt="境宏小幫手 QR"
            class="helper-qr-img"
            loading="lazy">
        </div>

        <div class="helper-qr-caption mt-3">
          掃描加入群組，每天 07:00 回報檢驗狀況
        </div>
